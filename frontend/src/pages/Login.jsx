...
function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(
        apiError.response?.data?.message || 'Unable to login. Please try again.',
      )
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-lg border border-stone-200 bg-white p-6 shadow-xl shadow-stone-200/70 md:p-8">
        ...
        <h1 id="login-title" className="mb-6 text-3xl font-bold text-stone-950">
          Login
        </h1>
        ...
        <form className="grid gap-3" onSubmit={handleSubmit}>
          <label className="text-sm font-semibold text-stone-800" htmlFor="login-email">
            Email
          </label>
          <input
        ...
        <p className="mt-6 text-sm text-stone-600">
          Need an account?{' '}
          <Link className="font-bold text-emerald-800 hover:text-emerald-950" to="/register">
            Create one
          </Link>
        </p>
      </div>
